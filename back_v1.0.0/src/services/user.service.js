...
// 내 정보 수정, 비밀번호 변경
const updateUserInfo = async (user_id, nickname, phoneNum, adress, password, newPassword, newPasswordConfirm) => {
  if (!user_id) {
    throw new CustomError('사용자 정보가 없습니다. 로그인을 다시 시도해주세요', 412);
  }
  ...
  if (password) {
    ...
    const updateUser = await userRepository.updateUserInfo(user_id, nickname, phoneNum, adress);
